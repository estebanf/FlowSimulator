...
var build_activity = function(name,setup){
  if(!setup) setup = {};
  ...
  if(!setup.conditional){
    setup.conditional = false; //default to no condition
  }
  else{
    if(!setup.conditional_target_true) throw "Bad setup - Missing conditional_target_true"
    if(!setup.conditional_target_false) throw "Bad setup - Missing conditional_target_false"
    if(!setup.prob_conditional) setup.prob_conditional = 0.5;
  }
  var activity = function(err,value){
    ...
    setTimeout(function(){
      endTime = new Date();
      elapsed = Math.round((endTime.getTime() - startTime.getTime()) / 1000);
      if(!value){
        value = []
      };
      value.push({
      ...
      if(!setup.loop && !setup.conditional){
        self(err,value);
      }
      else{
        if(setup.loop){
          ...
          self.conditional(function(){
            return prob_check(setup.prob_conditional);
          },setup.conditional_target_true,setup.conditional_target_false,value);
        }
      }
    },duration);    
  }
